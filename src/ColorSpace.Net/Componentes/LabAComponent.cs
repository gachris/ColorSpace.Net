...
/// <summary>
/// Represents a component for the A value in the Lab color space.
/// </summary>
internal class LabAComponent : NormalComponent
{
    ...
    /// <inheritdoc/>
    public override string Name => "Lab_A";
    ...
    public override byte[] GenerateNormalMapFromColor(Color color, int width, int height, int stride)
    {
        ...
        {
            var rgb = Lab.FromLab(lab.L, (int)iRowCurrent, lab.B).ToRgb(Illuminants.D65_2);
    ...
    {
        var pixels = new byte[stride * height];
        var iRowUnit = 100.0 / width;
        var iColUnit = 255.0 / width;
        var iRowCurrent = 100.0;
        var a = normalComponentValue;
        var index = 0;
    ...
    public override Color ColorAtPoint(Point point, int colorComponentValue)
    {
        ...
        var b = point.X - 128;
        var rgb = Lab.FromLab((int)l, a, b).ToRgb(Illuminants.D65_2);
